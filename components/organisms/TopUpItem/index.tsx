interface TopUpItemPros {
  typeLayar: "desktop" | "mobile";
  data: {
    name: string;
    thumbnail: string;
    category: {
...
export default function TopUpItem(props: TopUpItemPros) {
  const { typeLayar, data } = props;
  const API_IMAGE = process.env.NEXT_PUBLIC_IMAGE;
  if (typeLayar === "desktop") {
    return (
